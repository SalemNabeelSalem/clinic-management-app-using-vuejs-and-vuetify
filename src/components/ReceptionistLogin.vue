  ...
  <v-app>
    <v-layout align-center justify-center>
      <v-flex xs12 sm8 md3>
        ...
        <v-card>
          <v-card-title>Receptionist Login</v-card-title>
          <v-divider></v-divider>
          <v-card-text>
...
export default {
  ...
  methods: {
    login() {
      LoginDataService.receptionistLogin(this.email, this.password)
        .then((response) => {
          console.log(response.data);
          if (response.status == 200) {
            this.userId = response.data.id;
            this.$router.push(
              "/recep-dashboard/" + this.userId + "/patients-reservations"
            );
          }
        })
        .catch((error) => {
