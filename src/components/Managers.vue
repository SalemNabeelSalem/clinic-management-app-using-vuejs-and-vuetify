    ...
    >
      <template v-slot:top>
        ...
        <v-toolbar flat>
          ...
          <v-dialog v-model="dialog" max-width="500px">
            ...
            <v-card v-if="formTitle === 'New Manager'">
              <v-card-title>
            ...
            <v-card v-if="formTitle === 'Edit Manager'">
              <v-card-title>
                <span class="text-h5">{{ formTitle }}</span>
              </v-card-title>
      ...
      <template slot="item.switch" slot-scope="{ item }">
        <v-switch
          ...
          color="success"
          @click.stop="setManagerActivity(item.id, !item.isActive)"
        ></v-switch>
      </template>
...
<script>
// @ is an alias to /src
import ManagerDataService from "@/libs/ManagerDataService.js";
...
export default {
  ...
  methods: {
    ...
    loadRecords() {
      ManagerDataService.findAllManagers()
        .then((response) => {
    ...
    newRecord(item) {
      console.log("NEW MANAGER REQUEST: ");

      console.log(item);

      ManagerDataService.createNewManager(item)
        .then((response) => {
          this.loadRecords();

          console.log("NEW MANAGER RESPONSE: ");
    ...
    deleteItemConfirm() {
      ...
      ManagerDataService.deleteManager(deletedItem.id)
        .then((response) => {
          this.loadRecords();
    ...
    save() {
      if (this.editedIndex > -1) {
        ...
        console.log("UPDATED MANAGER REQUEST: ");

        console.log(this.customerEdit);

        ManagerDataService.updateManager(updatedItem.id, this.customerEdit)
          .then((response) => {
            this.loadRecords();

            console.log("UPDATED MANAGER RESPONSE: ");
        ...
        console.log("THE NEW MANAGER DATA: " + this.newItem);
      }
      this.close();
    },

    setManagerActivity(managerId, activityValue) {
      if (activityValue === false) {
        ManagerDataService.deactivateManager(managerId)
          .then((response) => {
            this.loadRecords();

            console.log("DEACTIVATE MANAGER WITH ID: " + managerId);
        ...
        ManagerDataService.activateManager(managerId)
          .then((response) => {
            this.loadRecords();

            console.log("ACTIVATE MANAGER WITH ID: " + managerId);
